...
void main(void)
{
    ...
    err = DIO_LineConfig(card, P7443_TTL0, 0, INPUT_PORT); // set Line0 of TTL0 to input
    if(err!=NoError) {
        printf("TTL Port0 Input Config Error: %d\n",err);
        exit(1);
    }

    err = DIO_LineConfig(card, P7443_TTL0, 1, OUTPUT_PORT); // set Line1 of TTL0 to output
    if(err!=NoError) {
        printf("TTL Port0 Output Config Error: %d\n",err);
        exit(1);
    }

    err = DIO_LinesConfig(card, P7443_TTL1, 0x1); // set Line0 of TTL1 to output
    if(err!=NoError) {
    ...
    do{
        ...
        err = DO_WriteLine(card, P7443_TTL0, 1, 0);
        if (err!=NoError) {
        ...
        err = DO_WriteLine(card, P7443_TTL1, 0, 1);
        if (err!=NoError) {
            printf("DO_WriteLine error=%d", err);
            exit(1);
        }
        ...
        err = DI_ReadLine(card, P7443_TTL1, 1, &value);
        if (err!=NoError) {
            printf("DO_WriteLine error=%d", err);
            exit(1);
        }
        ...
        err = DI_ReadLine(card, P7443_TTL0, 0, &value);
        if (err!=NoError) {
            printf("DO_WriteLine error=%d", err);
            exit(1);
        }
        ...
        err = DO_WriteLine(card, P7443_TTL0, 1, 1);
        if (err!=NoError) {
            printf("DO_WriteLine error=%d", err);
            exit(1);
        }
        ...
        err = DO_WriteLine(card, P7443_TTL1, 0, 0);
        if (err!=NoError) {
            printf("DO_WriteLine error=%d", err);
            exit(1);
        }
