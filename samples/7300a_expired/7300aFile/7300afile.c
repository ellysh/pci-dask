...
int main(void)
{
    ...
    err=DI_7300A_Config(card, 16, TRIG_CLK_10MHZ, P7300_WAIT_NO, P7300_TERM_ON, 0, 1, 1);
    if (err!=0) {
       printf("DI_7300A_Config error=%d\n", err);
       exit(1);
    }

    err=DO_7300A_Config (card, 16, TRIG_CLK_10MHZ, P7300_WAIT_NO, P7300_TERM_ON, 0);
    if (err!=0) {
