...
int main( void )
{
    ...
    setbuf( stdout, NULL );
    printf("PCI-9118 Double Buffered DMA with Pre-Trigger Mode\n");
    ...
    err=AI_9118_Config(card,P9118_AI_BiPolar|P9118_AI_SingEnded,P9118_AI_DtrgPositive|P9118_AI_EtrgPositive|P9118_AI_AboutTrgEn,0,0x1);
    if( err != NoError) {
    ...
    fprintf( fp, "PCI-9118 Double Buffered DMA with Pre-Trigger Mode\n");
    fprintf( fp, "count_per_page:%li\n", read_count );
    fprintf( fp, "channel_num:1\n" );
    fprintf( fp, "sample_rate(Hz):%li\n", (U32)sample_rate );
    fprintf( fp, "CH0\n======================\n");
    printf("\n\n\n Data conversion start , Input the Trigger Signal for Pre-Trigger operation\n");
