...
void main(void)
{
    ...
    err = DIO_PortConfig(card, P7442_TTL0, OUTPUT_PORT);
    if(err!=NoError) {
        printf("Input Port Config Error: %d\n",err);
        exit(1);
    }

    err = DIO_PortConfig(card, P7442_TTL1, INPUT_PORT);
    if(err!=NoError) {
    ...
    do{
        ...
        err = DO_WritePort(card, P7442_TTL0, out_value);
        if (err!=NoError) {
        ...
        err = DI_ReadPort(card, P7442_TTL1, &in_value);
        if (err!=NoError) {
