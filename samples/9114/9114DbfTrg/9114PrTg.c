...
U32 read_count = 2048, postCount = 512;
...
void * ProcessThread( void * temp_pt )
{
    ...
    fprintf( fp, " ContReadChannel with Double Buffer Transfer \n\n\n");
    fprintf( fp, "CH0  \n======================\n");
...
int main(void)
{
    ...
    setbuf( stdout, NULL );
    clrscr();
    printf("\n 9114(A) Double Buffer with PreTrigger : \n");
    printf(" This program inputs data from CH-%d of PCI-9114 in %d Hz by\n double-buffer mode, and store data to file '%s'. The size of circular\n buffer is %ld. It will not stop until the trigger signal or you press a key.\n\n Press any key to start the operation.\n", channel, (int)sample_rate, file_name,read_count);
    getch();
    ...
    err = AI_9114_PreTrigConfig(card, 1, postCount);
    if (err!=0) {
        printf("AI_PreTrigConfig error=%d", err);
        Release_Card(card);
        exit(1);
    }
    ...
    printf("\n\n\n\n\n Data conversion start , Insert the trigger signal or Press any key to stop input operation.\n");
