...
int main(void)
{
    ...
    DIO_PortConfig(card,Channel_P3A, OUTPUT_PORT);
    DIO_PortConfig(card,Channel_P1B, INPUT_PORT);
    DIO_PortConfig(card,Channel_P3B, OUTPUT_PORT);
    DIO_PortConfig(card,Channel_P1C, INPUT_PORT);
    DIO_PortConfig(card,Channel_P3C, OUTPUT_PORT);
    clrscr();
    printf("PCI-7296 DIO Sample :\n\n");
    printf("-------------------------------------------------\n");
    printf(" P1A, p1B, P1C are configured as input port.\n");
    printf(" P3A, p3B, P3C are configured as output port.\n");
    printf("-------------------------------------------------\n");
    ...
    do {
        ...
        printf(" P1A, p1B, P1C are configured as input port.\n");
        printf(" P3A, p3B, P3C are configured as output port.\n\n");
        DO_WritePort(card, Channel_P3A, out_value);
        printf(" P3A output = 0x%x\n", (U16)out_value);
        DI_ReadPort(card,  Channel_P1A, &input);
        printf(" P1A input = 0x%x\n", (U16)input);
        DO_WritePort(card, Channel_P3B, out_value);
        printf(" P3B output = 0x%x\n", (U16)out_value);
        DI_ReadPort(card,  Channel_P1B, &input);
        printf(" P1B input = 0x%x\n", (U16)input);
        DO_WritePort(card, Channel_P3C, out_value);
        printf(" P3C output = 0x%x\n", (U16)out_value);
