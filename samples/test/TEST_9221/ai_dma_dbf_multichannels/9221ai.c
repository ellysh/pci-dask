...
//    memset(DataStore, '\0', BufSize*10*2);
    ...
    err = AI_ContReadMultiChannels(card, NumChans, Chans, AdRanges, &BufferId1, 0, 0.0, ASYNCH_OP);  
    if(err < 0){
...
/*    if(ViewIndex){
	...
	memcpy(DataStore+Loop*BufSize, Buffer2, AccessCnt*2);
    }
    else{
        printf("Loop: %d, ViewIndex: %d\n", Loop, ViewIndex);
        memcpy(DataStore+Loop*BufSize, Buffer1, AccessCnt*2);
    }
...
/*
    for(i=0; i<BufSize*10/NumChans; i++){
	for(j=0; j<NumChans; j++){
            err = AI_VoltScale(card, AdRanges[j], DataStore[i*NumChans+j], &voltage);
	    if(err < 0)
