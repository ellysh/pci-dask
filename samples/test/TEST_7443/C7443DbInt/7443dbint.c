...
void sig1_handler( int signo )
{
	...
	DI_ReadPort(card, P7443_CH0, &CosData);
	printf("ReadData: %x\n", ~CosData);
	return;
}
...
int main(void)
{
    ...
    err = DIO_SetCOSInterrupt32(card, P7443_CH0, 0x3, NULL/*sig1_handler*/);
    if( err != NoError ){
       printf(" error from DIO_SetCOSInterrupt32 : %d \n" , err );
       goto exit_after_error;
    }
err = DIO_SetDualInterrupt(card, INT1_COS0, -1, sig1_handler, NULL);
printf("err: %d\n", err);


/*
    err = DIO_SetCOSInterrupt32(card, P7443_CH1, 0x0010, sig2_handler);
    if( err != NoError ){
       printf(" error from DIO_SetCOSInterrupt32 : %d \n" , err );
       goto exit_after_error;
    }

    err = DIO_SetCOSInterrupt32(card, P7443_CH2, 0x0100, sig3_handler);
    if( err != NoError ){
       printf(" error from DIO_SetCOSInterrupt32 : %d \n" , err );
       goto exit_after_error;
    }

    err = DIO_SetCOSInterrupt32(card, P7443_CH3, 0x1000, sig4_handler);
    if( err != NoError ){
       printf(" error from DIO_SetCOSInterrupt32 : %d \n" , err );
       goto exit_after_error;
    }
...
err = DIO_SetDualInterrupt(card, -1, -1, NULL, NULL);
printf("err: %d\n", err);
